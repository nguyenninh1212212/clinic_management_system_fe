      ...
      <Card className="print:shadow-none print:border-none">
        <CardContent className="p-8 sm:p-12 space-y-6 font-serif">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-slate-500 font-sans">
                  Địa chỉ: 123 Đường Y Tế, Phường Đa Khoa, TP. Hồ Chí Minh
                </p>
                <p className="text-xs text-slate-500 font-sans">
                  Hotline cấp cứu: 1900 8888 · Giấy phép hoạt động: 0123/BYT-GPHĐ
                </p>
              </div>
            </div>
            <div className="text-right text-xs font-sans text-slate-500">
              <div>
                Mã đơn: <strong className="font-mono text-slate-900">#{prescription.id}</strong>
              </div>
          ...
          <div className="text-center py-2">
            ...
            <p className="text-xs text-slate-500 font-sans">(Dùng cho bệnh nhân điều trị ngoại trú)</p>
          </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-xs font-sans p-4 bg-slate-50 rounded-lg">
            <div>
              ...
              <div className="font-bold text-slate-900 text-sm mt-0.5">{patient?.fullName}</div>
            </div>
            <div>
              <span className="text-slate-500">Giới tính / Tuổi:</span>
              <div className="font-semibold text-slate-800 mt-0.5">
                {patient?.gender} · {patient?.dateOfBirth ? `${dayjs().diff(patient.dateOfBirth, 'year')} tuổi` : '—'}
              </div>
            </div>
            <div>
            ...
            <div className="col-span-2 sm:col-span-4">
              ...
              <span className="text-slate-800 ml-2">{patient?.address || '—'}</span>
            </div>
            <div className="col-span-2 sm:col-span-4 pt-1 border-t border-slate-200">
              ...
              <span className="text-slate-900 font-bold ml-2">
                {exam?.diagnosis} {exam?.icd10Code && `(Mã ICD-10: ${exam.icd10Code})`}
              </span>
            </div>
          </div>
                  ...
                  <tr key={item.id} className="align-top">
                    ...
                    <td className="py-3 pr-2">
                      ...
                      <div className="text-[11px] text-slate-500">
                        {item.medicine?.genericName && `Hoạt chất: ${item.medicine.genericName}`}
                      </div>
                    </td>
                    ...
                    <td className="py-3">
                      <div className="font-semibold text-slate-800">
                        ...
                        {item.durationDays ? ` · Dùng ${item.durationDays} ngày` : ''}
                      </div>
                      {item.instructions && (
          ...
          <div className="grid grid-cols-2 pt-8 text-center text-xs font-sans text-slate-700">
            <div>
              <p className="font-medium text-slate-500">Bệnh nhân / Người nhà</p>
            ...
            <div>
              <p className="text-slate-500">
                Ngày {dayjs(prescription.createdAt).format('DD')} tháng {dayjs(prescription.createdAt).format('MM')} năm {dayjs(prescription.createdAt).format('YYYY')}
              </p>
              <p className="font-bold text-slate-900 mt-0.5">Bác sĩ khám bệnh</p>
              <p className="italic text-[10px] text-slate-400">(Ký và ghi rõ họ tên)</p>
              <div className="h-20" />
              <p className="font-bold text-slate-900">{doctor?.user?.fullName || 'BS. Chuyên khoa'}</p>
              <p className="text-[10px] text-slate-500 font-mono">CCHN: {doctor?.licenseNumber}</p>
            </div>
          </div>
        </CardContent>
      </Card>
