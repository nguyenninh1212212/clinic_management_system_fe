...
export const DoctorShiftForm: React.FC<DoctorShiftFormProps> = ({
  doctorId,
  defaultDate,
  existingShifts,
  isSaving,
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    if (!workDate || workDate < dayjs().format("YYYY-MM-DD"))
      return setError("Không thể phân công ca trong ngày đã qua.");
    if (!start.isValid() || !end.isValid() || !start.isBefore(end))
      return setError("Giờ bắt đầu phải nhỏ hơn giờ kết thúc.");
    setError("");
      ...
      <CardContent>
        ...
        >
          ...
          <Button
            ...
            variant="contained"
            startIcon={<AddIcon />}
            disabled={!doctorId || isSaving}
          >
            {isSaving ? "Đang lưu..." : "Thêm ca"}
          </Button>
          {error && (
            <Alert severity="warning" className="md:col-span-4">
              {error}
            </Alert>
          )}
        </form>
